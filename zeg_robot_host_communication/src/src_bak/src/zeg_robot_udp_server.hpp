#ifndef SRC_ZEG_ROBOT_zeg_robot_udp_server_HPP_
#define SRC_ZEG_ROBOT_zeg_robot_udp_server_HPP_
#include <vector>
#include <thread>
#include <memory>
...
public:
	...
	virtual int send(const char *buf, int len) override {
		return sendto(sock_fd_, buf, len, 0, (struct sockaddr *)&zeg_robot_udp_thread::scheduler_client_addr_, sizeof(zeg_robot_udp_thread::scheduler_client_addr_));
	}
...
private:
	...
	inline void make_broadcast_addr() {
		...
		broadcast_addr_.sin_port = htons(9780);
...
#endif /* SRC_ZEG_ROBOT_zeg_robot_udp_server_HPP_ */
